...
class PostFunctions {
	...
	public function get_author_vcard( $args = '' ) {
		...
		$avatar = get_avatar( $author_id, 50, null, esc_html( get_the_author_meta( 'display_name', $author_id ) ), array(
			'class' => 'photo u-photo avatar xbergr',
			'extra_attr' => 'itemprop="image"',
		) );

		$author = sprintf(
			/* translators: %s: Name post author */
			esc_html_x( '%s', 'post author', '@@textdomain' ),
			'<span class="author themeberger-author p-author vcard hcard h-card" itemprop="author" itemscope itemtype="http://schema.org/Person"><a class="url u-url" itemprop="url" rel="author" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID', $author_id ) ) ) . '">' . $avatar . '<span class="name p-name fn" itemprop="name">' . esc_html( get_the_author_meta( 'display_name', $author_id ) ) . '</span></a></span>'
		);
	...
	public function get_permalink_date( $args = '' ) {
		...
		$permalink_before = sprintf( $permalink_before,
		...
		if ( ! $args['human_readable'] ) {
			$permalink = $this->get_post_date( array(
				'before' => $permalink_before,
				'after' => $permalink_after
			) );
		} else {
			$permalink = $this->get_human_post_date( array(
				'before' => $permalink_before,
				'after' => $permalink_after
			) );
		}
	...
	public function get_first_video_of_post( $args = '' ) {
        ...
        if ( empty( $first_video ) ) {
			...
			$first_video = wp_video_shortcode( array( 'src' => $first_video, 'poster' => $poster, 'width' => 900 ) );
		}
	...
	public function get_content_without_first_video() {
		...
		*/
		...
		//$content = preg_replace( '~<div([^>]*)(class\\s*=\\s*["\']wp-video["\'])([^>]*)>(.*?)end_video -->~i', '', $content );
		// TODO: YouTube???
		//$content = preg_replace( '~<div([^>]*)(class\\s*=\\s*["\']video-embed["\'])([^>]*)>(.*?)</div>~i', '', $content );
