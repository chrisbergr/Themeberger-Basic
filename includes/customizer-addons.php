		...
		public function render_content() {
			...
			<label>
				...
				<button type="button"
					...
					aria-label="Reset"
					onclick="jQuery('#<?php echo $this->field_id ?>').val('<?php echo esc_attr( $this->value() ); ?>').trigger('input')"
				>Reset</button>
				<button type="button"
					class="button default"
					aria-label="Default"
					onclick="jQuery('#<?php echo $this->field_id ?>').val('<?php echo $this->field_default ?>').trigger('input')"
				>Default</button>
			</label>
			<?php
		}
